...
results = pickle.load(open('work_dirs/tps_ctw_e2e.pkl', 'rb'))
...
for i, img_id in enumerate(img_ids):
    ...
    recs = res['strs'] if 'strs' in res.keys() else None
    for j in range(len(ins)):
        pts = np.array(ins[j][:-1]).reshape(-1,2).tolist()
        score = ins[j][-1]
        rec = recs[j] if recs is not None else ''
        out = {
...
with open('tools/evaluation_det_e2e_offline_v3/ctw1500_res/my_text_results.json', 'w') as f:
    json.dump(output, f)
